#!/usr/bin/env python3
"""
Interactive CLI to run jailbreak detection tests on a REAL chatbot via CDP.
Tests prompts by sending them to the actual chatbot and grading responses.
"""
...
from tasks.single_turn import single_turn_jailbreak_detection
...
def interactive_mode():
    ...
    print("\n" + "=" * 50)
    print("CHATBOT JAILBREAK DETECTION TEST RUNNER")
    print("=" * 50)
    ...
    if ensure_chatbot_config() is None:
        return

    # Dataset selection (now returns a list)
    ...
    run_evaluation(dataset_arg, scorer_model, limit)




def run_evaluation(dataset, scorer_model, limit=10):
    ...
    if isinstance(dataset_arg, list):
        print(f"\nRunning evaluation on merged dataset ({len(dataset_arg)} files)...")
    else:
        print(f"\nRunning evaluation...")
    ...
    try:
        # Create task using single_turn_jailbreak_detection
        task = single_turn_jailbreak_detection(
...
def main():
    parser = argparse.ArgumentParser(
        description="Run jailbreak detection tests on a REAL chatbot via CDP (Chrome DevTools Protocol)"
    )
    parser.add_argument(
        "--chatbot-config",
        type=str,
    ...
    parser.add_argument(
        "--scorer-model",
        type=str,
        help="Model to use for scoring responses (default: anthropic/claude-sonnet-5)",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    if (
        ...
        and datasets
        and args.scorer_model
    ):
        dataset_arg = datasets[0] if len(datasets) == 1 else datasets
        run_evaluation(dataset_arg, args.scorer_model, limit)
    # If all required arguments provided without --non-interactive flag
    elif datasets and args.scorer_model:
        dataset_arg = datasets[0] if len(datasets) == 1 else datasets
        run_evaluation(dataset_arg, args.scorer_model, limit)
    # Otherwise, run interactive mode
    else:
        if args.dataset or args.scorer_model or args.limit != 10:
            print(
